<div class="container-fluid main-contents">
    ...
    <header class="header">
        <h1 id="contents_title">
            <?php echo $view_data['user_profile']->name; ?>さんが，<?php echo ($view_data['type'] === \Config::get('TYPE_LEND'))?'貸しているリスト' : '借りているリスト';?>
        </h1>        
    </header>
    <?php if (count($view_data['lend_and_borrow_list']) > 0): ?>
        <section class="row-fluid">
            <?php foreach($view_data['lend_and_borrow_list'] as $lend_and_borrow_list): ?>
                <a href="<?php echo \Uri::base() . 'share/lendandborrow/edit_data/'. $view_data['type'] . '/' . $lend_and_borrow_list['id'];?>">
                    <div>
